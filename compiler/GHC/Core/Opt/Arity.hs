...
-- See Note [Combining arityType with demand info]
...
* We want to eta-reduce if doing so leaves
  ...
  *including* a cast.  For example
  ...
  See Note [Which RHSs do we eta-expand?] in GHC.Core.Opt.Simplify.Utils.
  If we eta-reduce to a PAP, we don't want to eta-expand PAPs!
